...
export function SocialIconLinks({
  ...
  if (variant === 'buttons') {
    return (
      <div
        className={cn(
          'flex flex-wrap items-center justify-center gap-2 sm:gap-3 w-full min-w-0 social-buttons-wrap',
          className
        )}
        role="group"
      ...
      >
        {BUTTON_ITEMS.map((item) => (
          <a
            ...
            className={cn(
              'inline-flex items-center justify-center gap-2 h-10 rounded-full border-2 bg-white px-3 py-2 sm:px-4 sm:py-2 text-sm font-medium text-slate-900 transition-colors flex-1 min-w-[11rem] max-w-xs',
              item.key === 'telegram' && 'social-button-telegram',
              item.borderClass,
