...
export function Header() {
  ...
  return (
    <header className="static z-50 w-full overflow-visible border-b border-slate-200 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80">
      <Topbar />

      {/* Основная строка: логотип и навигация на всю ширину. */}
      <Container className="header-container w-full min-w-0 max-w-full 2xl:max-w-[1760px] px-4 sm:px-6 lg:px-8">
        <div className="grid h-20 min-h-20 w-full min-w-0 grid-cols-[auto_minmax(0,1fr)] items-center gap-4 sm:h-[6rem] sm:min-h-[6rem] sm:gap-6 min-[1260px]:gap-6 xl:gap-8">
          {/* Логотип */}
          <Link
            href="/"
            className="flex shrink-0 min-w-0 items-center overflow-hidden"
          ...
          >
            <Image
              ...
              width={340}
              height={102}
              className="h-12 w-auto max-w-[180px] object-contain object-left sm:h-14 sm:max-w-[220px] xl:h-[4.5rem] xl:max-w-none"
              priority
            />
          </Link>
