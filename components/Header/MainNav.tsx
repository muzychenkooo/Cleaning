...
const linkBase =
  'shrink-0 whitespace-nowrap rounded-md px-5 py-2.5 text-base font-medium transition-colors nav-link-base';
const linkInactive =
...
function IndividualsMegaMenu({ pathname }: { pathname: string }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'absolute left-0 top-full z-[1000] w-max pt-2 transition-all duration-200',
          open
            ? 'pointer-events-auto translate-y-0 opacity-100'
            : 'pointer-events-none -translate-y-1 opacity-0'
        )}
      >
        <div className="rounded-xl border border-slate-200 bg-white py-2 shadow-xl min-w-[200px] max-w-[calc(100vw-2rem)]">
...
function BusinessMegaMenu({ pathname }: { pathname: string }) {
  ...
  const [open, setOpen] = React.useState(false);
  const [dropdownMaxWidth, setDropdownMaxWidth] = React.useState<number | undefined>(undefined);
  const triggerRef = React.useRef<HTMLDivElement>(null);
  const closeTimeoutRef = React.useRef<number | null>(null);

  const updateDropdownMaxWidth = React.useCallback(() => {
    if (typeof window === 'undefined' || !triggerRef.current) return;
    const rect = triggerRef.current.getBoundingClientRect();
    const max = Math.min(48 * 16, window.innerWidth - rect.left - 16);
    setDropdownMaxWidth(Math.max(200, max));
  }, []);

  React.useEffect(() => {
    if (!open) return;
    updateDropdownMaxWidth();
    window.addEventListener('resize', updateDropdownMaxWidth);
    return () => window.removeEventListener('resize', updateDropdownMaxWidth);
  }, [open, updateDropdownMaxWidth]);
  ...
  return (
    ...
    >
      ...
      {/* Left edge under parent item; max-width constrained so right edge does not overflow viewport */}
      <div
        className={cn(
          'absolute left-0 top-full z-[1000] w-max pt-2 transition-all duration-200',
          open
      ...
      >
        <div
          className="max-h-[70vh] overflow-y-auto rounded-2xl border border-slate-200 bg-white px-6 py-5 shadow-xl"
          style={open ? { maxWidth: `${dropdownMaxWidth ?? (typeof window !== 'undefined' ? Math.min(768, Math.max(200, window.innerWidth - 32)) : 736)}px` } : undefined}
        >
...
export function MainNav() {
  ...
  return (
    <nav
      className="nav-main relative hidden min-[1260px]:flex w-full min-w-0 flex-1 flex-nowrap items-center justify-end gap-2 overflow-visible"
