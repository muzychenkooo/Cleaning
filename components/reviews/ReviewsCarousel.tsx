...
export function ReviewsCarousel() {
  ...
  // ── Card width: use fixed px to prevent grid from forcing document overflow ───
  const colWidthPx = containerW > 0 ? (containerW - (cardsPerView - 1) * GAP) / cardsPerView : 280;
  const colWidth = `${colWidthPx}px`;
  ...
  return (
    <div className="min-w-0 w-full max-w-full overflow-x-clip">
      ...
      <div
        ref={containerRef}
        className="carousel-contain mt-6 min-w-0 w-full max-w-full touch-pan-y"
        style={{ width: '100%' }}
